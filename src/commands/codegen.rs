...
pub async fn run(
    ...
    for codegen in config.codegen.iter() {
        ...
        match &codegen.plugin {
            // Built-in clients are generated by tx3c itself: no download, and the
            // templates always match the installed tx3c.
            CodegenPlugin::Known(plugin) => {
                for (name, tii_path) in &targets {
                    ...
                    crate::spawn::tx3c::codegen_language(tii_path, plugin.tx3c_language(), &dest)?;
