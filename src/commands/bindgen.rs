...
const TEMPLATE_TYPESCRIPT_PACKAGEJSON: &str = include_str!("templates/gen-typescript/package.json.tpl");
const TEMPLATE_TYPESCRIPT_TESTTS: &str = include_str!("templates/gen-typescript/test.ts.tpl");
const TEMPLATE_TYPESCRIPT_TSCONFIGJSON: &str = include_str!("templates/gen-typescript/tsconfig.json.tpl");
...
fn generate_typescript(job: &tx3_bindgen::Job, version: &str) -> miette::Result<()> {
    ...
    std::fs::write(
        ...
        TEMPLATE_TYPESCRIPT_TESTTS
            .replace("{{protocol_name}}", &job.name)
    ).into_diagnostic()?;
    std::fs::write(dest_path.join("tsconfig.json"), TEMPLATE_TYPESCRIPT_TSCONFIGJSON).into_diagnostic()?;
...
pub fn run(_args: Args, config: &Config) -> miette::Result<()> {
    for bindgen in config.bindings.iter() {
        ...
        match bindgen.plugin.as_str() {
            ...
            "typescript" => generate_typescript(&job, &config.protocol.version)?,
